...
<div id="sidr-container">
            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">Course Name</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        <h1>Course Name Table</h1>
        <div class="stroke" style="width:10%;"></div>
        <?php
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            echo "<form action=\"display_course.php\" method=\"post\">
                <p>CCRN: <input type=\"text\" placeholder=\"crn\" name=\"ccrn\" value=\"\"></p>
                <p>Course Name: <input type=\"text\" name=\"cterm\" value=\"\"></p>
                <p>Course Number: <input type=\"text\" name=\"cumber\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            $cumber = $conn->real_escape_string($_POST['cumber']);
            $cterm = $conn->real_escape_string($_POST['cterm']);
            $ccrn = $conn->real_escape_string($_POST['ccrn']);

            if ($cnumber !== "" && $cname !== "" && $ccrn !== "") {
                $sql = "insert into `f19_team5`.`course` ( `ccrn`,`cterm`,`cumber`) values ('$ccrn','$cterm','$cumber')";
                $result = $conn->query($sql);
            }
            if ($_POST["Delete"] !== "") {
                $sql = "delete from f19_team5.course where sid = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
            displayCourse();
