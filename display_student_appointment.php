...
<div id="sidr-container">
            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">Student Appointment</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        <h1>Student Appointment Table</h1>
        <div class="stroke" style="width:10%;"></div>
        <?php
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            echo "<form action=\"display_student_appointment.php\" method=\"post\">
                <p>Uemail: <input type=\"text\" placeholder=\"id\" name=\"uemail\" value=\"\"></p>
                <p>Sid: <input type=\"text\" placeholder=\"id\" name=\"sid\" value=\"\"></p>
                <p>Atime <input type=\"text\" name=\"atime\" value=\"\"></p>
                <p>Adate: <input type=\"text\" name=\"adate\" value=\"\"></p>
                <p>SAnotes <input type=\"text\" name=\"sanotes\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            $sid = $conn->real_escape_string($_POST['sid']);
            $uemail = $conn->real_escape_string($_POST['uemail']);
            $atime = $conn->real_escape_string($_POST['atime']);
            $adate = $conn->real_escape_string($_POST['adate']);
            $sanotes = $conn->real_escape_string($_POST['sanotes']);

            if ($sid !== "" && $uemail !== "") {
                $sql = "insert into `f19_team5`.`student_appointment` (`uemail`, `atime`, `adate`, `sanotes`,`sid`) values ('$uemail','$atime','$adate', '$sanotes', '$sid')";
                $result = $conn->query($sql);
            }
            if ($_POST["Delete"] !== "") {
                $sql = "delete from f19_team5.student_appointment where uemail = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
            displayStudentAppointment();
        } else {
            echo "Access denied.";
        }
        ?>
