...
<div id="sidr-container">
            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">User</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        ...
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            echo "<form action=\"display_front_desk.php\" method=\"post\">
                <p>Email: <input type=\"text\" placeholder=\"id\" name=\"uemail\" value=\"\"></p>
                <p>Password: <input type=\"text\" name=\"fdpassword\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            $uemail = $conn->real_escape_string($_POST['uemail']);
            $fdpassword = $conn->real_escape_string(hash('sha256', $_POST['fdpassword']));
            if ($uemail !== "" && $fdpassword !== "") {
                $sql = "insert into `f19_team5`.`front_desk` (`uemail`, `fdpassword`) values ('$uemail','$fdpassword')";
                $result = $conn->query($sql);
            }
            if ($_POST["delete"] !== "") {
                $sql = "delete from f19_team5.front_desk where uemail = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
            displayFrontDesk();
