...
<div id="sidr-container">
            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">Courses Taken</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        <h1>Courses taken Table</h1>
        <div class="stroke" style="width:10%;"></div>
        <?php
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            echo "<form action=\"display_student_courses_taken.php\" method=\"post\">
                <p>Student ID: <input type=\"text\" placeholder=\"id\" name=\"sid\" value=\"\"></p>
                <p>Curses Taken: <input type=\"text\" name=\"sctaken\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            $sid = $conn->real_escape_string($_POST['sid']);
            $sctaken = $conn->real_escape_string($_POST['sctaken']);
            if ($sid !== "") {
                $sql = "insert into `f19_team5`.`student_courses_taken` ( `sid`,`sctaken`) values ('$sid','$sctaken')";
                $result = $conn->query($sql);
            }
            if ($_POST["Delete"] !== "") {
                $sql = "delete from f19_team5.student_courses_taken where sid = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
