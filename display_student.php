...
<div id="sidr-container">
            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">Student Table</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        ...
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            echo "<form action=\"display_student.php\" method=\"post\">
                <p>Uemail: <input type=\"text\" placeholder=\"id\" name=\"uemail\" value=\"\"></p>
                <p>Sid: <input type=\"text\" placeholder=\"id\" name=\"sid\" value=\"\"></p>
                <p>Spassword: <input type=\"text\" name=\"spassword\" value=\"\"></p>
                <p>SDeg Plan: <input type=\"text\" name=\"sdeg_plan\" value=\"\"></p>
                <p>Classification: <input type=\"text\" name=\"sclassification\" value=\"\"></p>
                <p>SGrad Date: <input type=\"text\" name=\"sgrad_date\" value=\"\"></p>
                <p>SGpa: <input type=\"text\" name=\"sgpa\" value=\"\"></p>
                <p>SFirst Name: <input type=\"text\" name=\"sfirst_name\" value=\"\"></p>
                <p>SMiddle Name: <input type=\"text\" name=\"smiddle_name\" value=\"\"></p>
                <p>SLast Name: <input type=\"text\" name=\"slast_name\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            $id = $conn->real_escape_string($_POST['sid']);
            $uemail = $conn->real_escape_string($_POST['uemail']);
            $spass = $conn->real_escape_string(hash('sha256', $_POST['spassword']));
            $plan = $conn->real_escape_string($_POST['sdeg_plan']);
            $class = $conn->real_escape_string($_POST['sclassification']);
            $grad = $conn->real_escape_string($_POST['sgrad_date']);
            $gpa = $conn->real_escape_string($_POST['sgpa']);
            $fn = $conn->real_escape_string($_POST['sfirst_name']);
            $mn = $conn->real_escape_string($_POST['smiddle_name']);
            $ln = $conn->real_escape_string($_POST['slast_name']);
            if ($id !== "" && $spass !== "") {
                $sql = "insert into `f19_team5`.`student` (`uemail`, `sid`, `spassword`, `sdeg_plan`,`sclassification`, `sgrad_date`, `sgpa`, `sfirst_name`, `smiddle_name`,`slast_name`) values ('$uemail','$id','$spass', '$plan', '$class','$grad', '$gpa', '$fn', '$mn','$ln')";
                $result = $conn->query($sql);
            }
            if ($_POST["delete"] !== "") {
                $sql = "delete from f19_team5.student where uemail = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
            displayStudent();
