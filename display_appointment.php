...
<div id="sidr-container">
            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">Appointment Table</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        ...
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            echo "<form action=\"display_appointment.php\" method=\"post\">
                <p>Appointment Date: <input type=\"text\"  name=\"adate\" value=\"\"></p>
                <p>Atime <input type=\"text\" name=\"atime\" value=\"\"></p>
                <p>Alocation: <input type=\"text\" name=\"alocation\" value=\"\"></p>
                <p>aterm <input type=\"text\" name=\"aterm\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            $adate = $conn->real_escape_string($_POST['adate']);
            $atime = $conn->real_escape_string($_POST['atime']);
            $alocation = $conn->real_escape_string($_POST['alocation']);
            $aterm = $conn->real_escape_string($_POST['aterm']);

            if ($adate !== "") {
                $sql = "insert into `f19_team5`.`student_appointment` (`adate`, `atime`, `alocation`, `aterm`) values ('$adate','$atime','$alocation', '$aterm')";
                $result = $conn->query($sql);
            }
            if ($_POST["Delete"] !== "") {
                $sql = "delete from f19_team5.student_appointment where sid = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
            displayAppointment();
