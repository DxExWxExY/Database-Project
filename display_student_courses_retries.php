...
<div id="sidr-container">
            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">Course Retried</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        <h1>Student Courses Retries Table</h1>
        ...
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            ...
            $scretries = $conn->real_escape_string($_POST['scretries']);
            echo "<form action=\"display_student_courses_retries.php\" method=\"post\">
                <p>Student ID: <input type=\"text\" placeholder=\"id\" name=\"sid\" value=\"\"></p>
                <p>Curses Retries: <input type=\"text\" name=\"scretries\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            if ($sid !== "") {
                $sql = "insert into `f19_team5`.`student_courses_retries` ( `sid`,`scretries`) values ('$sid','$scretries')";
                $result = $conn->query($sql);
            }
            if ($_POST["Delete"] !== "") {
                $sql = "delete from f19_team5.student_courses_retries where sid = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
            displayStudentCoursesRetries();
        } else {
            echo "Access denied.";
        }
        ?>
