...
<div id="sidr-container">
            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">Advising Slip</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        <h1>Advising Slip Table</h1>
        <div class="stroke" style="width:10%;"></div>
        <?php
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            echo "<form action=\"display_advising_slip.php\" method=\"post\">
                <p>Student ID: <input type=\"text\"  name=\"sid\" value=\"\"></p>
                <p>Approval<input type=\"text\" name=\"asapproval\" value=\"\"></p>
                <p>Date <input type=\"text\" name=\"asdate\" value=\"\"></p>
                <p>Term <input type=\"text\" name=\"asterm\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            $sid = $conn->real_escape_string($_POST['sid']);
            $asapproval = $conn->real_escape_string($_POST['asapproval']);
            $asdate = $conn->real_escape_string($_POST['asdate']);
            $asterm = $conn->real_escape_string($_POST['asterm']);

            if ($sid !== "") {
                $sql = "insert into `f19_team5`.`advising_slip` (`sid`, `asapproval`, `asdate`, `asterm`) values ('$sid','$asapproval','$asdate', '$asterm')";
                $result = $conn->query($sql);
            }
            if ($_POST["Delete"] !== "") {
                $sql = "delete from f19_team5.advising_slip where sid = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
            displayAdvisingSlip();
