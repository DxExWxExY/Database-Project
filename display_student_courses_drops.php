...
<div id="sidr-container">
            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">Courses Drops</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        <h1>Student Courses Drops Table</h1>
        <div class="stroke" style="width:10%;"></div>
        <?php
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            echo "<form action=\"display_student_courses_drops.php\" method=\"post\">
                <p>Student ID: <input type=\"text\" placeholder=\"id\" name=\"sid\" value=\"\"></p>
                <p>Curses Drops: <input type=\"text\" name=\"scdrops\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            $sid = $conn->real_escape_string($_POST['sid']);
            $scdrops = $conn->real_escape_string($_POST['scdrops']);

            if ($sid !== "") {
                $sql = "insert into `f19_team5`.`student_courses_drops` ( `sid`,`scdrops`) values ('$sid','$scdrops')";
                $result = $conn->query($sql);
            }
            if ($_POST["Delete"] !== "") {
                $sql = "delete from f19_team5.student_courses_drops where sid = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
            displayStudentCoursesDrops();
        } else {
            echo "Access denied.";
        }
        ?>
