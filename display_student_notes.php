...
<div id="sidr-container">
            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">Notes</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        ...
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            echo "<form action=\"display_student_notes.php\" method=\"post\">
                <p>Asid: <input type=\"text\" placeholder=\"id\" name=\"asid\" value=\"\"></p>
                <p>Snote: <input type=\"text\" name=\"snote\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            $asid = $conn->real_escape_string($_POST['asid']);
            $snote = $conn->real_escape_string($_POST['snote']);
            if ($sid !== "") {
                $sql = "insert into `f19_team5`.`advising_notes` ( `asid`,`snote`) values ('$asid','$snote')";
                $result = $conn->query($sql);
            }
            if ($_POST["Delete"] !== "") {
                $sql = "delete from f19_team5.advising_notes where asid = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
            displayStudentNotes();
