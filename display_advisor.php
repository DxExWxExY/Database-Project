            ...
            <div class="container headerContainer">
                ...
                <div class="UTEP-Branding">
                    ...
                    <div class="collegeHeading">
                        <h1 role="banner"><a href="#">Advisor Table</a></h1>
                    </div>
                    <!-- /navbar-header -->
                </div>
                <!-- /UTEP-Branding -->
    ...
    <div class="main-content">
        ...
        if (isset($_SESSION['login_user']) && $_SESSION['login_user'] === 'frontdesk') {
            echo "<form action=\"display_front_desk.php\" method=\"post\">
                ...
                <p>Password: <input type=\"text\" name=\"apassword\" value=\"\"></p>
                <input type=\"submit\" >
            </form>";
            $uemail = $conn->real_escape_string($_POST['uemail']);
            $apassword = $conn->real_escape_string(hash('sha256', $_POST['apassword']));
            if ($uemail !== "" && $apassword !== "") {
                $sql = "insert into `f19_team5`.`advisor` (`uemail`, `apassword`) values ('$uemail','$apassword')";
                $result = $conn->query($sql);
            }
            if ($_POST["delete"] !== "") {
                $sql = "delete from f19_team5.advisor where uemail = '" . $_POST["delete"] . "'";
                $result = $conn->query($sql);
                $_POST["delete"] = "";
            }
            displayAdvisor();
